...
class UserInterfaceStockPredictions(GeneralUI):
    ...
    def avaliable_num_neurons(self) -> int:
        st.write(
            "# Chose number of neurons in the first and second hidden layer of the MLP"
        )
...
class ControllerStockPredictions:
    ...
    def _build_forcast_factory(self):
        ...
        model_parameters = initializer.generate_model_parameters(
            self._architecture, self._learning_rate, batch_size=self._batch_size
        )
        ...
        self._forcast_factory.predict(self._num_predictions, end_date=self._date_str)
